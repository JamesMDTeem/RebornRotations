...
public sealed class PCT_Default : PictomancerRotation
{
    ...
    private bool ExecuteOpener(out IAction? act)
    {
        ...
        while (openerStep < opener.Length)
        {
            if (CanUseAction(opener[openerStep], out act))
            {
                openerStep++;
                return true;
            }
    ...
    private bool ExecuteSingleMuseBurst(out IAction? act)
    {
        ...
        var burst = new IAction[]
        {
            ...
            ThunderInMagentaPvE,
            CometInBlackPvE,
            PomMusePvE, // Assuming this is the Creature Muse in the diagram
            StarPrismPvE,
            HammerStampPvE,
            HammerBrushPvE,
            PolishingHammerPvE,
        ...
        while (burstStep < burst.Length)
        {
            if (CanUseAction(burst[burstStep], out act))
            {
                burstStep++;
                return true;
            }
    ...
    private bool CanUseAction(IAction action, out IAction? act)
    {
        act = null;

        // Check if the action is a BaseAction and can be used
        if (action is BaseAction baseAction)
        {
            return baseAction.CanUse(out act, skipAoeCheck: true);
        }

        // Add other checks here if needed for different action types
